...
{%- set payment_methods = get_column_values(ref('t_domain_nielsen_fact_payments'), 'PAYMENT_METHOD', 'ASC') -%}
...
SELECT
    ...
    {% endfor %}
FROM
    {{ ref('t_domain_nielsen_fact_payments') }} PAYMENTS
    INNER JOIN {{ ref('t_domain_nielsen_fact_orders') }} ORDERS ON (ORDERS.ORDER_SK = PAYMENTS.ORDER_SK)
GROUP BY
    1
